 ...
 * @author tprog084
 */
public class IngresoMonedero extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try{
            if(null!=request.getParameter("Saldo")){
                ...
                usuProxy.ingresarSaldoUsuario(s.getId(),saldo);
                WebServices.UsuarioWS.DataUsuario dataU = usuProxy.obtenerUsuarioLogueado(s.getId());
                s.setAttribute("usuario",dataU);
                System.out.println("pago exitoso");
            }

            String acciones="window.close();";
            response.getOutputStream().println("<script language=\"JavaScript\">"+acciones+"</script>");
